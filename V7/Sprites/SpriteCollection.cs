...
namespace V7
{
    public class SpriteCollection : List<Sprite>, IDisposable
    {
        public void Dispose()
        {
            ...
            List<Sprite> dispose = FindAll((s) => s is InstancedSprite);
            foreach (Sprite sprite in dispose)
            {
                sprite.Dispose();
            }
        }
        ...
        public virtual void Render(int frame, bool showInvisible = false, float progress = 1)
        {
            if (!Visible) return;
            ...
            if (tiles.Count > 0 && tileTexture is object)
            {
                ...
                GL.DrawArraysInstanced(PrimitiveType.TriangleFan, 0, 4, tiles.Count);
            }
            for (int i = 0; i < Count; i++)
            {
                ...
                double cx = d.DX, cy = d.DY;
                if (progress < 1)
                {
                    d.DX = d.PreviousX + (d.DX - d.PreviousX) * progress;
                    d.DY = d.PreviousY + (d.DY - d.PreviousY) * progress;
                }
                d.RenderPrep();
        ...
        private void SetBuffer()
        {
            tiles.Clear();
            tileTexture = null;
            List<Sprite> tls = FindAll((tl) => tl is Tile && tl.Layer < 0);
        ...
        public bool Contains(SpriteGroup group)
        {
            return group.AddedTo == this;
        }

        public void Add(SpriteGroup group)
        {
            group.AddToCollection(this);
        }
        public void Remove(SpriteGroup group)
        {
            group.RemoveFromCollection(this);
        }
