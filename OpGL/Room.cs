...
namespace V7
{
    public class Room : IDisposable
    {
        ...
        public static Room LoadRoom(JToken loadFrom, Game game, int xOffset = 0, int yOffset = 0)
        {
            ...
            ret.Color = Color.FromArgb((int)(loadFrom["Color"] ?? -1));
            ...
            if (ret.GroupName is object)
            {
                ...
                if (gName is null)
                    ...
                    ret.Ground = preset[gName].Ground.Initialize();
                ...
                if (bName is null)
                    ...
                    ret.Background = preset[bName].Background.Initialize();
                string sName = (string)loadFrom["SpikesName"];
                if (sName is null)
                    ret.Spikes = AutoTileSettings.Load(loadFrom["Spikes"]) ?? AutoTileSettings.Default4(8, 0);
                else
                    ret.Spikes = preset[sName].Spikes.Initialize();
            }
            Texture texture = game.TextureFromName((string)loadFrom["TileTexture"]);
